...
const instantiateHubContract = async (
  ...
  const command = `blkd tx wasm instantiate ${codeId} "${jsonString}" --label ${label} --admin ${adminAddress} --from admin --gas=5000000 --fees=1000000ushe -y --broadcast-mode block -o json`;
  const output = await execute(command);
  const response = JSON.parse(output);
