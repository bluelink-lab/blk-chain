...
import (
	...
	"github.com/she-protocol/she-chain/precompiles/addr"
...
func TestAssociatePubKey(t *testing.T) {
	...
	targetSheAddress, targetEvmAddress := testkeeper.PrivateKeyToAddresses(targetPrivKey)

	// Caller refers to the party calling the precompile.
	callerPrivKey := testkeeper.MockPrivateKey()
	callerSheAddress, callerEvmAddress := testkeeper.PrivateKeyToAddresses(callerPrivKey)
	...
	happyPathOutput, _ := associatePubKey.Outputs.Pack(targetSheAddress.String(), targetEvmAddress)
	...
	tests := []struct {
		...
		},
		{
			...
			wantErr:    true,
			wantErrMsg: fmt.Sprintf("address %s is already associated with evm address %s", callerSheAddress, callerEvmAddress),
		},
		{
...
func TestAssociate(t *testing.T) {
	...
	happyPathOutput, _ := associate.Outputs.Pack(targetSheAddress.String(), targetEvmAddress)
...
func TestGetAddr(t *testing.T) {
	...
	pre, _ := addr.NewPrecompile(k, k.BankKeeper(), k.AccountKeeper())
	getSheAddr, err := pre.ABI.MethodById(pre.GetExecutor().(*addr.PrecompileExecutor).GetSheAddressID)
	require.Nil(t, err)
	...
	getSheAddrBz, err := getSheAddr.Inputs.Pack(evmAddr)
	require.Nil(t, err)
	res, _, err := pre.RunAndCalculateGas(&vm.EVM{StateDB: stateDB}, evmAddr, evmAddr, append(getSheAddr.ID, getSheAddrBz...), 20000, common.Big0, nil, true, false)
	require.Nil(t, err)
	unpacked, err := getSheAddr.Outputs.Unpack(res)
	require.Nil(t, err)
	require.Equal(t, 1, len(unpacked))
	require.Equal(t, sheAddr.String(), unpacked[0].(string))

	getEvmAddrBz, err := getEvmAddr.Inputs.Pack(sheAddr.String())
	require.Nil(t, err)
