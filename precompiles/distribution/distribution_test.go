...
import (
	...
	"github.com/she-protocol/she-chain/precompiles/distribution"
...
func TestWithdraw(t *testing.T) {
	...
	require.Nil(t, k.BankKeeper().SendCoinsFromModuleToAccount(ctx, evmtypes.ModuleName, sheAddr, amt))
	...
	d, found := testApp.StakingKeeper.GetDelegation(ctx, sheAddr, val)
	require.True(t, found)
	require.Equal(t, int64(100), d.Shares.RoundInt().Int64())

	// set withdraw addr
	withdrawSheAddr, withdrawAddr := testkeeper.MockAddressPair()
	k.SetAddressMapping(ctx, withdrawSheAddr, withdrawAddr)
	...
	require.Nil(t, err)
	require.Empty(t, res.VmError)
	require.Equal(t, withdrawSheAddr.String(), testApp.DistrKeeper.GetDelegatorWithdrawAddr(ctx, sheAddr).String())
...
func setWithdrawAddressAndWithdraw(
	...
	// reinitialized
	for _, val := range vals {
		_, found := testApp.StakingKeeper.GetDelegation(ctx, sheAddr, val)
		require.True(t, found)
	}
}
...
func TestPrecompile_RunAndCalculateGas_WithdrawDelegationRewards(t *testing.T) {
	...
	validatorAddress := "shevaloper1reedlc9w8p7jrpqfky4c5k90nea4p6dhk5yqgd"
...
func TestPrecompile_RunAndCalculateGas_WithdrawMultipleDelegationRewards(t *testing.T) {
	_, notAssociatedCallerEvmAddress := testkeeper.MockAddressPair()
	_, contractEvmAddress := testkeeper.MockAddressPair()
	validatorAddresses := []string{"shevaloper1reedlc9w8p7jrpqfky4c5k90nea4p6dhk5yqgd"}
...
func TestPrecompile_RunAndCalculateGas_SetWithdrawAddress(t *testing.T) {
	_, notAssociatedCallerEvmAddress := testkeeper.MockAddressPair()
	callerSheAddress, callerEvmAddress := testkeeper.MockAddressPair()
	_, contractEvmAddress := testkeeper.MockAddressPair()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := testApp.NewContext(false, tmtypes.Header{}).WithBlockHeight(2)
			k := &testApp.EvmKeeper
			k.SetAddressMapping(ctx, callerSheAddress, callerEvmAddress)
			stateDb := state.NewDBImpl(ctx, k, true)
			evm := vm.EVM{
				StateDB:   stateDb,
	...
	rewards := []distrtypes.DelegationDelegatorReward{
		{
			...
			Reward: sdk.NewDecCoins(
				...
				sdk.NewDecCoin("ublk", sdk.NewInt(int64(val1ublkCoins))),
			),
		},
		{
			ValidatorAddress: "shevaloper16znh8ktn33dwnxxc9q0jmxmjf6hsz4tl0s6vxh",
			Reward:           sdk.NewDecCoins(sdk.NewDecCoin("ublk", sdk.NewInt(int64(val2ublkCoins)))),
		},
	}

	allDecCoins := sdk.NewDecCoins(sdk.NewDecCoin("uatom", sdk.NewInt(int64(uatomCoins))),
		sdk.NewDecCoin("ublk", sdk.NewInt(int64(val1ublkCoins+val2ublkCoins))))
