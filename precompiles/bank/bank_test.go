...
func TestRun(t *testing.T) {
	...
	require.Nil(t, err)
	err = k.BankKeeper().MintCoins(ctx, types.ModuleName, sdk.NewCoins(sdk.NewCoin("ushe", sdk.NewInt(10000000))))
	require.Nil(t, err)
	err = k.BankKeeper().SendCoinsFromModuleToAccount(ctx, types.ModuleName, senderAddr, sdk.NewCoins(sdk.NewCoin("ushe", sdk.NewInt(10000000))))
	require.Nil(t, err)
	...
	require.Nil(t, err)
	args, err := send.Inputs.Pack(senderEVMAddr, evmAddr, "ushe", big.NewInt(25))
	require.Nil(t, err)
	...
	// Send native 10_000_000_000_100, split into 10 ushe 100wei
	...
	var expectedEvts sdk.Events = []sdk.Event{
		...
		banktypes.NewCoinSpentEvent(senderAddr, sdk.NewCoins(sdk.NewCoin("ushe", sdk.NewInt(200000)))),
		// wei events
		...
		banktypes.NewCoinSpentEvent(senderAddr, sdk.NewCoins(sdk.NewCoin("ushe", sdk.NewInt(10)))),
		banktypes.NewCoinReceivedEvent(sheAddr, sdk.NewCoins(sdk.NewCoin("ushe", sdk.NewInt(10)))),
		sdk.NewEvent(
			banktypes.EventTypeTransfer,
			sdk.NewAttribute(banktypes.AttributeKeyRecipient, sheAddr.String()),
			sdk.NewAttribute(banktypes.AttributeKeySender, senderAddr.String()),
			sdk.NewAttribute(sdk.AttributeKeyAmount, sdk.NewCoin("ushe", sdk.NewInt(10)).String()),
		),
		sdk.NewEvent(
		...
		banktypes.NewCoinReceivedEvent(senderAddr, sdk.NewCoins(sdk.NewCoin("ushe", sdk.NewInt(132401)))),
		// tip is paid to the validator
		banktypes.NewCoinReceivedEvent(sdk.MustAccAddressFromBech32("she1v4mx6hmrda5kucnpwdjsqqqqqqqqqqqqlve8dv"), sdk.NewCoins(sdk.NewCoin("ushe", sdk.NewInt(67599)))),
	}
	require.EqualValues(t, expectedEvts.ToABCIEvents(), evts)

	// Use precompile balance to verify sendNative ushe amount succeeded
	balance, err := p.ABI.MethodById(p.GetExecutor().(*bank.PrecompileExecutor).BalanceID)
	require.Nil(t, err)
	args, err = balance.Inputs.Pack(evmAddr, "ushe")
	require.Nil(t, err)
	...
	require.Nil(t, err)
	require.Nil(t, k.BankKeeper().SendCoins(ctx, sheAddr, k.GetSheAddressOrDefault(ctx, p.Address()), sdk.NewCoins(sdk.NewCoin("ushe", sdk.OneInt()))))
...
func TestMetadata(t *testing.T) {
	...
	k.BankKeeper().SetDenomMetaData(ctx, banktypes.Metadata{Name: "SHE", Symbol: "ushe", Base: "ushe"})
	p, err := bank.NewPrecompile(k.BankKeeper(), bankkeeper.NewMsgServerImpl(k.BankKeeper()), k, k.AccountKeeper())
	require.Nil(t, err)
	statedb := state.NewDBImpl(ctx, k, true)
	evm := vm.EVM{
	...
	require.Nil(t, err)
	args, err := name.Inputs.Pack("ushe")
	require.Nil(t, err)
