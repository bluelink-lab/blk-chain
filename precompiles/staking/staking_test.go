...
func TestPrecompile_Run_Delegation(t *testing.T) {
	...
	validatorAddress := "shevaloper134ykhqrkyda72uq7f463ne77e4tn99steprmz7"
	...
	delegationResponse := &stakingtypes.QueryDelegationResponse{
		DelegationResponse: &stakingtypes.DelegationResponse{
			...
			Balance: sdk.NewCoin("ublt", sdk.NewInt(int64(shares))),
		},
	}
	...
	tests := []struct {
		...
		},
		{
			...
			fields: fields{
				stakingQuerier: &TestStakingQuerier{
					Err: fmt.Errorf("delegation with delegator %s not found for validator", callerSheAddress.String()),
				},
			},
			args: args{
				delegatorAddress: callerEvmAddress,
			...
			wantErr:    true,
			wantErrMsg: fmt.Sprintf("delegation with delegator %s not found for validator", callerSheAddress.String()),
		},
		{
