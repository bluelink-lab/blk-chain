...
func (p PrecompileExecutor) delegate(ctx sdk.Context, method *abi.Method, caller common.Address, args []interface{}, value *big.Int) ([]byte, error) {
	...
	delegator, associated := p.evmKeeper.GetSheAddress(ctx, caller)
	if !associated {
	...
	coin, err := pcommon.HandlePaymentUshe(ctx, p.evmKeeper.GetSheAddressOrDefault(ctx, p.address), delegator, value, p.bankKeeper)
	if err != nil {
		return nil, err
	}
...
func (p PrecompileExecutor) delegation(ctx sdk.Context, method *abi.Method, args []interface{}, value *big.Int) ([]byte, error) {
	...
	sheDelegatorAddress, err := pcommon.GetSheAddressFromArg(ctx, args[0], p.evmKeeper)
	if err != nil {
		return nil, err
	}
	...
	delegationRequest := &stakingtypes.QueryDelegationRequest{
		DelegatorAddr: sheDelegatorAddress.String(),
