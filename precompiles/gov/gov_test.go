...
func TestGovPrecompile(t *testing.T) {
	...
	tests := []struct {
		name           string
		args           args
		setup          func(ctx sdk.Context, k *keeper.Keeper, evmAddr common.Address, sheAddr sdk.AccAddress)
		verify         func(t *testing.T, ctx sdk.Context, sheAddr sdk.AccAddress, proposalID uint64)
		wantErr        bool
		avoidAssociate bool
	}{
		{
		...
		},
		{
			...
			verify: func(t *testing.T, ctx sdk.Context, sheAddr sdk.AccAddress, proposalID uint64) {
				v, found := testApp.GovKeeper.GetVote(ctx, proposalID, sheAddr)
		...
		},
		{
			...
			setup: func(ctx sdk.Context, k *keeper.Keeper, evmAddr common.Address, sheAddr sdk.AccAddress) {},
			verify: func(t *testing.T, ctx sdk.Context, sheAddr sdk.AccAddress, proposalID uint64) {
				_, found := testApp.GovKeeper.GetVote(ctx, proposalID, sheAddr)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tt.setup(ctx, k, evmAddr, sheAddr)
			...
			if tt.wantErr {
				...
				tt.verify(t, ctx, sheAddr, tt.args.proposal)
			}
		})
	}
}
