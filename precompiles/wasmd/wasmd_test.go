...
func TestExecute(t *testing.T) {
	...
	require.Equal(t, fmt.Sprintf("received test msg from %s with 1000ublk", mockAddr.String()), string(outputs[0].([]byte)))
	require.NotZero(t, g)
...
func TestExecuteBatchOneMessage(t *testing.T) {
	...
	require.Nil(t, err)
	require.Equal(t, 1, len(outputs))
	require.Equal(t, fmt.Sprintf("received test msg from %s with 1000ublk", mockAddr.String()), string((outputs[0].([][]byte))[0]))
	require.NotZero(t, g)
	require.Equal(t, int64(1000), testApp.BankKeeper.GetBalance(statedb.Ctx(), contractAddr, "ublt").Amount.Int64())
...
func TestExecuteBatchMultipleMessages(t *testing.T) {
	...
	require.Equal(t, fmt.Sprintf("received test msg from %s with 1000ublk", mockAddr.String()), string(parsedOutputs[0]))
	require.Equal(t, fmt.Sprintf("received test msg from %s with 1000ublk", mockAddr.String()), string(parsedOutputs[1]))
	require.Equal(t, fmt.Sprintf("received test msg from %s with 1000ublk", mockAddr.String()), string(parsedOutputs[2]))
	require.NotZero(t, g)
