...
func TransactionDependencyGenerator(_ aclkeeper.Keeper, evmKeeper evmkeeper.Keeper, ctx sdk.Context, msg sdk.Msg) ([]sdkacltypes.AccessOperation, error) {
	...
	sender := evmMsg.Derived.SenderSheAddr
	...
	toAddress := tx.To()
	if toAddress != nil {
		sheAddress := evmKeeper.GetSheAddressOrDefault(ctx, *toAddress)
		ops = appendRWBalanceOps(ops, sheAddress)
		ops = append(ops, sdkacltypes.AccessOperation{
			AccessType:         sdkacltypes.AccessType_READ,
			ResourceType:       sdkacltypes.ResourceType_KV_EVM_E2S,
			IdentifierTemplate: hex.EncodeToString(evmtypes.EVMAddressToSheAddressKey(*toAddress)),
		}, sdkacltypes.AccessOperation{
			AccessType:         sdkacltypes.AccessType_READ,
	...
	return append(ops, []sdkacltypes.AccessOperation{
		...
		},
		{
			...
			IdentifierTemplate: hex.EncodeToString(evmtypes.SheAddressToEVMAddressKey(evmKeeper.AccountKeeper().GetModuleAddress(authtypes.FeeCollectorName))),
		},
		{
			AccessType:         sdkacltypes.AccessType_READ,
			ResourceType:       sdkacltypes.ResourceType_KV_EVM_E2S,
			IdentifierTemplate: hex.EncodeToString(evmtypes.EVMAddressToSheAddressKey(evmAddr)),
		},
		{
			AccessType:         sdkacltypes.AccessType_READ,
			ResourceType:       sdkacltypes.ResourceType_KV_EVM_S2E,
			IdentifierTemplate: hex.EncodeToString(evmtypes.SheAddressToEVMAddressKey(evmMsg.Derived.SenderSheAddr)),
		},
		{
			AccessType:         sdkacltypes.AccessType_READ,
			ResourceType:       sdkacltypes.ResourceType_KV_EVM_CODE_HASH,
