...
func (m *MockQueryClient) SheAddressByEVMAddress(ctx context.Context, in *evmtypes.QuerySheAddressByEVMAddressRequest, opts ...grpc.CallOption) (*evmtypes.QuerySheAddressByEVMAddressResponse, error) {
	if in.EvmAddress == "0x0CF3Bb7Da9fea6a881987A5018740500C7d4BAaE" {
		return &evmtypes.QuerySheAddressByEVMAddressResponse{SheAddress: "", Associated: false}, nil
	}
	return &evmtypes.QuerySheAddressByEVMAddressResponse{SheAddress: "she1u8j4gaxyzhg39dk848q5w9h53tgggpcx74m762", Associated: true}, nil
}
...
func (m *MockErrorQueryClient) SheAddressByEVMAddress(ctx context.Context, in *evmtypes.QuerySheAddressByEVMAddressRequest, opts ...grpc.CallOption) (*evmtypes.QuerySheAddressByEVMAddressResponse, error) {
...
func Test_ParseAllowListJSON(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "EVM address error on getting SHE address",
		...
		},
		{
			...
			json: fmt.Sprintf(`{"addresses": ["%s", "%s", "%s"]}`, sheAddr1, sheAddr1, sheAddr2),
			want: banktypes.AllowList{Addresses: []string{sheAddr1, sheAddr2}},
		},
	}
