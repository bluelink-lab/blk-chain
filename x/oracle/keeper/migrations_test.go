...
func TestMigrate2to3(t *testing.T) {
	...
	store.Set(types.GetExchangeRateKey(utils.MicroSheDenom), bz)

	// validate legacy store value
	b := store.Get(types.GetExchangeRateKey(utils.MicroSheDenom))
	dp := sdk.DecProto{}
	...
	// Get rate
	rate, lastUpdate, lastUpdateTimestamp, err := input.OracleKeeper.GetBaseExchangeRate(input.Ctx, utils.MicroSheDenom)
