...
func TestExchangeRate(t *testing.T) {
	...
	// Set & get rates
	input.OracleKeeper.SetBaseExchangeRate(input.Ctx, utils.MicroSheDenom, cnyExchangeRate)
	rate, lastUpdate, _, err := input.OracleKeeper.GetBaseExchangeRate(input.Ctx, utils.MicroSheDenom)
	...
	input.OracleKeeper.SetVoteTarget(input.Ctx, utils.MicroSheDenom)
...
func TestIterateSheExchangeRates(t *testing.T) {
	input := CreateTestInput(t)
...
func TestVoteTargetGetSet(t *testing.T) {
	...
	voteTargets := map[string]types.Denom{
		...
		utils.MicroSheDenom:  {Name: utils.MicroSheDenom},
	}
