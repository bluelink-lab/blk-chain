...
1. All current active BLK exchange rates are purged from the store
...
4. For each remaining `denom` with a passing ballot:
    ...
    - Set the BLK exchange rate on the blockchain for that BLK<>`denom` with `k.SetSheExchangeRate()`
