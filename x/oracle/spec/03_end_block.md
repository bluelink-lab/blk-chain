...
1. All current active SHE exchange rates are purged from the store
...
4. For each remaining `denom` with a passing ballot:
    ...
    - Set the SHE exchange rate on the blockchain for that SHE<>`denom` with `k.SetSheExchangeRate()`
