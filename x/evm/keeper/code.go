...
func (k *Keeper) SetCode(ctx sdk.Context, addr common.Address, code []byte) {
	...
	// set association with direct cast BLK address for the contract address
	if _, ok := k.GetSheAddress(ctx, addr); !ok {
		k.SetAddressMapping(ctx, k.GetSheAddressOrDefault(ctx, addr), addr)
	}
}
...
func (k *Keeper) GetCodeHash(ctx sdk.Context, addr common.Address) common.Hash {
	...
	if bz == nil {
		...
		if k.GetBalance(ctx, k.GetSheAddressOrDefault(ctx, addr)).Cmp(utils.Big0) == 0 {
			return common.Hash{}
		}
