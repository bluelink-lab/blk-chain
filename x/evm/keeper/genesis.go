...
func (k *Keeper) InitGenesis(ctx sdk.Context, genState types.GenesisState) {
	...
	sheAddrFc := k.accountKeeper.GetModuleAddress(authtypes.FeeCollectorName) // feeCollector == coinbase
	k.SetAddressMapping(ctx, sheAddrFc, GetCoinbaseAddress())

	for _, addr := range genState.AddressAssociations {
		k.SetAddressMapping(ctx, sdk.MustAccAddressFromBech32(addr.SheAddress), common.HexToAddress(addr.EthAddress))
	}
