...
func (q Querier) SheAddressByEVMAddress(c context.Context, req *types.QuerySheAddressByEVMAddressRequest) (*types.QuerySheAddressByEVMAddressResponse, error) {
	ctx := sdk.UnwrapSDKContext(c)
	if req.EvmAddress == "" {
		return nil, sdkerrors.ErrInvalidRequest
	}
	evmAddr := common.HexToAddress(req.EvmAddress)
	addr, found := q.Keeper.GetSheAddress(ctx, evmAddr)
	if !found {
		return &types.QuerySheAddressByEVMAddressResponse{Associated: false}, nil
	}

	return &types.QuerySheAddressByEVMAddressResponse{SheAddress: addr.String(), Associated: true}, nil
}

func (q Querier) EVMAddressBySheAddress(c context.Context, req *types.QueryEVMAddressBySheAddressRequest) (*types.QueryEVMAddressBySheAddressResponse, error) {
	ctx := sdk.UnwrapSDKContext(c)
	if req.SheAddress == "" {
		return nil, sdkerrors.ErrInvalidRequest
	}
	sheAddr, err := sdk.AccAddressFromBech32(req.SheAddress)
	if err != nil {
		return nil, err
	}
	addr, found := q.Keeper.GetEVMAddress(ctx, sheAddr)
	if !found {
		return &types.QueryEVMAddressBySheAddressResponse{Associated: false}, nil
	}

	return &types.QueryEVMAddressBySheAddressResponse{EvmAddress: addr.Hex(), Associated: true}, nil
}
