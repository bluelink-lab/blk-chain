...
import (
	...
	shedbtypes "github.com/she-protocol/she-db/ss/types"
...
type Keeper struct {
	...
	receiptStore shedbtypes.StateStore
...
func NewKeeper(
	storeKey sdk.StoreKey, transientStoreKey sdk.StoreKey, paramstore paramtypes.Subspace, receiptStateStore shedbtypes.StateStore,
...
func (k *Keeper) PrepareReplayedAddr(ctx sdk.Context, addr common.Address) {
	...
	if a.Balance != nil && a.Balance.Cmp(utils.Big0) != 0 {
		ublt, wei := state.SplitUsheWeiAmount(a.Balance)
		err = k.BankKeeper().AddCoins(ctx, k.GetSheAddressOrDefault(ctx, addr), sdk.NewCoins(sdk.NewCoin("ublt", ublt)), true)
		if err != nil {
			panic(err)
		}
		err = k.BankKeeper().AddWei(ctx, k.GetSheAddressOrDefault(ctx, addr), wei)
		if err != nil {
			panic(err)
		}
	}
