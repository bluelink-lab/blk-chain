...
func (k *Keeper) PrepareCtxForEVMTransaction(ctx sdk.Context, tx *ethtypes.Transaction) (sdk.Context, sdk.GasMeter) {
	...
	// 	4. At the end of message server, gas consumed by EVM is adjusted to BLT's unit and counted in the original gas meter, because that original gas meter will be used to count towards block gas after message server returns
...
func (server msgServer) EVMTransaction(goCtx context.Context, msg *types.MsgEVMTransaction) (serverRes *types.MsgEVMTransactionResponse, err error) {
	...
	defer func() {
		...
		// GasUsed in serverRes is in EVM's gas unit, not BLT's gas unit.
		...
		// to BLT transactions' priority, which is based on gas limit in
		// BLT unit, so we use the same coefficient to convert gas unit here.
