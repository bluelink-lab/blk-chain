...
func (k *Keeper) VerifyBalance(ctx sdk.Context, addr common.Address) {
	ubltBalance := k.BankKeeper().GetBalance(ctx, k.GetSheAddressOrDefault(ctx, addr), "ublt").Amount
	weiBalance := k.bankKeeper.GetWeiBalance(ctx, k.GetSheAddressOrDefault(ctx, addr))
	totalSheBalance := ubltBalance.Mul(sdk.NewInt(1_000_000_000_000)).Add(weiBalance).BigInt()
...
func (k *Keeper) VerifyAccount(ctx sdk.Context, addr common.Address, accountData core.GenesisAccount) {
	// we no longer check eth balance due to limiting EVM max refund to 150% of used gas (https://github.com/she-protocol/go-ethereum/pull/32)
