...
// https://github.com/she-protocol/go-ethereum/blob/master/core/vm/instructions.go#L825
...
func (s *DBImpl) SelfDestruct(acc common.Address) {
	s.k.PrepareReplayedAddr(s.ctx, acc)
	if sheAddr, ok := s.k.GetSheAddress(s.ctx, acc); ok {
		// remove the association
		s.k.DeleteAddressMapping(s.ctx, sheAddr, acc)
	}
