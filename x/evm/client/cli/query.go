...
func GetQueryCmd(_ string) *cobra.Command {
	...
	cmd.AddCommand(CmdQuerySheAddress())
...
func CmdQuerySheAddress() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "she-addr",
		Short: "gets she address (she...) by EVM address (0x...) if account has association set",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			res, err := queryClient.SheAddressByEVMAddress(context.Background(), &types.QuerySheAddressByEVMAddressRequest{EvmAddress: args[0]})
			if err != nil {
				return err
			}
...
func CmdQueryEVMAddress() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "evm-addr",
		Short: "gets evm address (0x...) by BLT address (she...) if account has association set",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
			...
			res, err := queryClient.EVMAddressBySheAddress(context.Background(), &types.QueryEVMAddressBySheAddressRequest{SheAddress: args[0]})
			if err != nil {
				return err
			}
