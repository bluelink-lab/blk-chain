...
import (
	...
	"github.com/bluelink-lab/blk-chain/x/evm/client/wasm"
...
func TestHandleERC721RoyaltyInfo(t *testing.T) {
	...
	require.Nil(t, err)
	require.NotEmpty(t, res2)
	match, _ := regexp.MatchString(`{"receiver":"she\w{39}","royalty_amount":"5"}`, string(res2))
...
func TestGetAddress(t *testing.T) {
	...
	k.SetAddressMapping(ctx, sheAddr1, evmAddr1)
	...
	res, err := h.HandleGetEvmAddress(ctx, sheAddr1.String())
	require.Nil(t, err)
	...
	getEvmAddrResp = &bindings.GetEvmAddressResponse{}
	res, err = h.HandleGetEvmAddress(ctx, sheAddr2.String())
	require.Nil(t, err)
	require.Nil(t, json.Unmarshal(res, getEvmAddrResp))
	require.True(t, getEvmAddrResp.Associated)
	getSheAddrResp := &bindings.GetSheAddressResponse{}
	res, err = h.HandleGetSheAddress(ctx, evmAddr1.Hex())
	require.Nil(t, err)
	require.Nil(t, json.Unmarshal(res, getSheAddrResp))
	require.True(t, getSheAddrResp.Associated)
	require.Equal(t, sheAddr1.String(), getSheAddrResp.SheAddress)
	getSheAddrResp = &bindings.GetSheAddressResponse{}
	res, err = h.HandleGetSheAddress(ctx, evmAddr2.Hex())
	require.Nil(t, err)
	require.Nil(t, json.Unmarshal(res, getSheAddrResp))
	require.True(t, getSheAddrResp.Associated)
}
