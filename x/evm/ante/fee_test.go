...
func TestEVMFeeCheckDecorator(t *testing.T) {
	...
	amt := typedTx.Cost()
	coinsAmt := sdk.NewCoins(sdk.NewCoin(k.GetBaseDenom(ctx), sdk.NewIntFromBigInt(amt).Quo(sdk.NewIntFromBigInt(state.UsheToSweiMultiplier)).Add(sdk.OneInt())))
	k.BankKeeper().MintCoins(ctx, types.ModuleName, coinsAmt)
	sheAddr := sdk.AccAddress(msg.Derived.SenderSheAddr)
	k.BankKeeper().SendCoinsFromModuleToAccount(ctx, types.ModuleName, sheAddr, coinsAmt)
