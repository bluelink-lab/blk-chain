...
func (p *EVMPreprocessDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (sdk.Context, error) {
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(evmtypes.EventTypeSigner,
		...
		sdk.NewAttribute(evmtypes.AttributeKeySheAddress, sheAddr.String())))
	...
	_, isAssociated := p.evmKeeper.GetEVMAddress(ctx, sheAddr)
	if isAssociateTx && isAssociated {
		...
		if !p.IsAccountBalancePositive(ctx, sheAddr, evmAddr) {
			metrics.IncrementAssociationError("associate_tx_insufficient_funds", evmtypes.NewAssociationMissingErr(sheAddr.String()))
			return ctx, sdkerrors.Wrap(sdkerrors.ErrInsufficientFunds, "account needs to have at least 1 wei to force association")
		}
		if err := associateHelper.AssociateAddresses(ctx, sheAddr, evmAddr, pubkey); err != nil {
			return ctx, err
		}
...
func (p *EVMPreprocessDecorator) IsAccountBalancePositive(ctx sdk.Context, sheAddr sdk.AccAddress, evmAddr common.Address) bool {
	baseDenom := p.evmKeeper.GetBaseDenom(ctx)
	if amt := p.evmKeeper.BankKeeper().GetBalance(ctx, sheAddr, baseDenom).Amount; amt.IsPositive() {
		return true
	}
	if amt := p.evmKeeper.BankKeeper().GetBalance(ctx, sdk.AccAddress(evmAddr[:]), baseDenom).Amount; amt.IsPositive() {
		return true
	}
	if amt := p.evmKeeper.BankKeeper().GetWeiBalance(ctx, sheAddr); amt.IsPositive() {
		return true
	}
...
func Preprocess(ctx sdk.Context, msgEVMTransaction *evmtypes.MsgEVMTransaction) error {
	...
	if atx, ok := txData.(*ethtx.AssociateTx); ok {
		...
		evmAddr, sheAddr, pubkey, err := helpers.GetAddresses(V, R, S, customMessageHash)
		if err != nil {
			return err
		}
		msgEVMTransaction.Derived = &derived.Derived{
			SenderEVMAddr: evmAddr,
			SenderSheAddr: sheAddr,
	...
	evmAddr, sheAddr, shePubkey, err := helpers.GetAddresses(V, R, S, txHash)
	if err != nil {
		return err
	}
	msgEVMTransaction.Derived = &derived.Derived{
		SenderEVMAddr: evmAddr,
		SenderSheAddr: sheAddr,
		PubKey:        &secp256k1.PubKey{Key: shePubkey.Bytes()},
		Version:       version,
		IsAssociate:   false,
	}
	return nil
...
func (p *EVMPreprocessDecorator) AnteDeps(txDeps []sdkacltypes.AccessOperation, tx sdk.Tx, txIndex int, next sdk.AnteDepGenerator) (newTxDeps []sdkacltypes.AccessOperation, err error) {
	...
	return next(append(txDeps, sdkacltypes.AccessOperation{
		...
		IdentifierTemplate: hex.EncodeToString(evmtypes.SheAddressToEVMAddressKey(msg.Derived.SenderSheAddr)),
		...
		IdentifierTemplate: hex.EncodeToString(evmtypes.EVMAddressToSheAddressKey(msg.Derived.SenderEVMAddr)),
	}, sdkacltypes.AccessOperation{
		AccessType:         sdkacltypes.AccessType_READ,
		ResourceType:       sdkacltypes.ResourceType_KV_BANK_BALANCES,
		IdentifierTemplate: hex.EncodeToString(banktypes.CreateAccountBalancesPrefix(msg.Derived.SenderSheAddr)),
	}, sdkacltypes.AccessOperation{
		AccessType:         sdkacltypes.AccessType_WRITE,
		ResourceType:       sdkacltypes.ResourceType_KV_BANK_BALANCES,
		IdentifierTemplate: hex.EncodeToString(banktypes.CreateAccountBalancesPrefix(msg.Derived.SenderSheAddr)),
		...
		IdentifierTemplate: hex.EncodeToString(authtypes.AddressStoreKey(msg.Derived.SenderSheAddr)),
	}, sdkacltypes.AccessOperation{
		AccessType:         sdkacltypes.AccessType_WRITE,
		ResourceType:       sdkacltypes.ResourceType_KV_AUTH_ADDRESS_STORE,
		IdentifierTemplate: hex.EncodeToString(authtypes.AddressStoreKey(msg.Derived.SenderSheAddr)),
...
func (p *EVMAddressDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (sdk.Context, error) {
	...
	for _, signer := range signers {
		if evmAddr, associated := p.evmKeeper.GetEVMAddress(ctx, signer); associated {
			ctx.EventManager().EmitEvent(sdk.NewEvent(evmtypes.EventTypeSigner,
				sdk.NewAttribute(evmtypes.AttributeKeyEvmAddress, evmAddr.Hex()),
				sdk.NewAttribute(evmtypes.AttributeKeySheAddress, signer.String())))
			continue
		}
