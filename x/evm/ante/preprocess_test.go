...
func TestPreprocessAnteHandler(t *testing.T) {
	...
	require.Nil(t, k.BankKeeper().AddCoins(ctx, sdk.AccAddress(evmAddr[:]), sdk.NewCoins(sdk.NewCoin("ublk", sdk.NewInt(100))), true))
	...
	require.Nil(t, err)
	require.Equal(t, sdk.AccAddress(privKey.PubKey().Address()), sdk.AccAddress(msg.Derived.SenderSheAddr))
	require.Equal(t, sdk.NewInt(100), k.BankKeeper().GetBalance(ctx, sheAddr, "ublk").Amount)
	require.Equal(t, sdk.NewInt(10), k.BankKeeper().GetWeiBalance(ctx, sheAddr))
	require.Equal(t, sdk.ZeroInt(), k.BankKeeper().GetBalance(ctx, sdk.AccAddress(evmAddr[:]), "ublk").Amount)
...
func TestPreprocessAssociateTx(t *testing.T) {
	...
	sheAddr := sdk.AccAddress(privKey.PubKey().Address())
	...
	k.BankKeeper().SendCoinsFromModuleToAccount(ctx, types.ModuleName, sheAddr, amt)
	ctx, err = handler.AnteHandle(ctx, mockTx{msgs: []sdk.Msg{msg}}, false, func(ctx sdk.Context, _ sdk.Tx, _ bool) (sdk.Context, error) {
		panic("should not be called")
	})
	require.Nil(t, err)
	associated, ok := k.GetEVMAddress(ctx, sheAddr)
	require.True(t, ok)
...
func TestPreprocessAssociateTxWithWeiBalance(t *testing.T) {
	...
	require.NotNil(t, err)
	k.BankKeeper().AddWei(ctx, sheAddr, sdk.OneInt())
	ctx, err = handler.AnteHandle(ctx, mockTx{msgs: []sdk.Msg{msg}}, false, func(ctx sdk.Context, _ sdk.Tx, _ bool) (sdk.Context, error) {
		panic("should not be called")
	})
	require.Nil(t, err)
...
func TestEVMAddressDecorator(t *testing.T) {
	...
	msg := banktypes.NewMsgSend(sender, recipient, sdk.NewCoins(sdk.NewCoin("ublk", sdk.OneInt())))
...
func TestEVMAddressDecoratorContinueDespiteErrors(t *testing.T) {
	...
	msg := banktypes.NewMsgSend(sender, sender, sdk.NewCoins(sdk.NewCoin("ublk", sdk.OneInt())))   // to self to simplify
	ctx, err = handler.AnteHandle(ctx, mockTx{msgs: []sdk.Msg{msg}, signers: []sdk.AccAddress{sender}}, false, func(ctx sdk.Context, _ sdk.Tx, _ bool) (sdk.Context, error) {
		return ctx, nil
	})
...
func TestMigrateBalance(t *testing.T) {
	...
	k.BankKeeper().AddCoins(ctx, sdk.AccAddress(evmAddr[:]), sdk.NewCoins(sdk.NewCoin("ublk", sdk.NewInt(2))), false)
	...
	k.AccountKeeper().SetAccount(ctx, vestingtypes.NewDelayedVestingAccountRaw(
		vestingtypes.NewBaseVestingAccount(
			...
			sdk.NewCoins(sdk.NewCoin("ublk", sdk.NewInt(1))), math.MaxInt64, admin),
	))
	associateHelper := helpers.NewAssociationHelper(k, k.BankKeeper(), k.AccountKeeper())
	require.Nil(t, associateHelper.MigrateBalance(ctx, evmAddr, sheAddr))
	require.Equal(t, int64(1), k.BankKeeper().SpendableCoins(ctx, sheAddr).AmountOf("ublk").Int64())
	require.Equal(t, int64(0), k.BankKeeper().LockedCoins(ctx, sheAddr).AmountOf("ublk").Int64())
	require.Equal(t, int64(0), k.BankKeeper().SpendableCoins(ctx, sdk.AccAddress(evmAddr[:])).AmountOf("ublk").Int64())
	require.Equal(t, int64(1), k.BankKeeper().LockedCoins(ctx, sdk.AccAddress(evmAddr[:])).AmountOf("ublk").Int64())
}
