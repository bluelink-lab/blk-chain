...
func TestPreprocessAnteHandler(t *testing.T) {
	...
	require.Nil(t, k.BankKeeper().AddCoins(ctx, sdk.AccAddress(evmAddr[:]), sdk.NewCoins(sdk.NewCoin("ublt", sdk.NewInt(100))), true))
	...
	require.Equal(t, sdk.NewInt(100), k.BankKeeper().GetBalance(ctx, sheAddr, "ublt").Amount)
	require.Equal(t, sdk.NewInt(10), k.BankKeeper().GetWeiBalance(ctx, sheAddr))
	require.Equal(t, sdk.ZeroInt(), k.BankKeeper().GetBalance(ctx, sdk.AccAddress(evmAddr[:]), "ublt").Amount)
...
func TestEVMAddressDecorator(t *testing.T) {
	...
	msg := banktypes.NewMsgSend(sender, recipient, sdk.NewCoins(sdk.NewCoin("ublt", sdk.OneInt())))
...
func TestEVMAddressDecoratorContinueDespiteErrors(t *testing.T) {
	...
	msg := banktypes.NewMsgSend(sender, sender, sdk.NewCoins(sdk.NewCoin("ublt", sdk.OneInt())))   // to self to simplify
	ctx, err = handler.AnteHandle(ctx, mockTx{msgs: []sdk.Msg{msg}, signers: []sdk.AccAddress{sender}}, false, func(ctx sdk.Context, _ sdk.Tx, _ bool) (sdk.Context, error) {
		return ctx, nil
	})
...
func TestMigrateBalance(t *testing.T) {
	...
	k.BankKeeper().AddCoins(ctx, sdk.AccAddress(evmAddr[:]), sdk.NewCoins(sdk.NewCoin("ublt", sdk.NewInt(2))), false)
	...
	k.AccountKeeper().SetAccount(ctx, vestingtypes.NewDelayedVestingAccountRaw(
		vestingtypes.NewBaseVestingAccount(
			...
			sdk.NewCoins(sdk.NewCoin("ublt", sdk.NewInt(1))), math.MaxInt64, admin),
	))
	...
	require.Equal(t, int64(1), k.BankKeeper().SpendableCoins(ctx, sheAddr).AmountOf("ublt").Int64())
	require.Equal(t, int64(0), k.BankKeeper().LockedCoins(ctx, sheAddr).AmountOf("ublt").Int64())
	require.Equal(t, int64(0), k.BankKeeper().SpendableCoins(ctx, sdk.AccAddress(evmAddr[:])).AmountOf("ublt").Int64())
	require.Equal(t, int64(1), k.BankKeeper().LockedCoins(ctx, sdk.AccAddress(evmAddr[:])).AmountOf("ublt").Int64())
}
