...
func TestSimple(t *testing.T) {
	...
	require.Nil(t, k.BankKeeper().SendCoinsFromModuleToAccount(ctx, types.ModuleName, sheAddr, amt))
	require.Nil(t, k.BankKeeper().MintCoins(ctx, types.ModuleName, sdk.NewCoins(sdk.NewCoin("test", sdk.NewInt(200000000)))))
	require.Nil(t, k.BankKeeper().SendCoinsFromModuleToAccount(ctx, types.ModuleName, sheAddr, sdk.NewCoins(sdk.NewCoin("test", sdk.NewInt(200000000)))))
	...
	_, found := k.GetSheAddress(ctx, common.HexToAddress(receipt.ContractAddress))
