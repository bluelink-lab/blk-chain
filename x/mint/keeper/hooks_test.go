...
func TestEndOfEpochMintedCoinDistribution(t *testing.T) {
	...
	t.Run("Initial should be zero", func(t *testing.T) {
		sheApp := keepertest.TestApp()
		ctx := sheApp.BaseApp.NewContext(false, tmproto.Header{Time: time.Now()})

		header := tmproto.Header{
			Height: sheApp.LastBlockHeight() + 1,
			Time:   time.Now().UTC(),
		}
		sheApp.BeginBlock(ctx, abci.RequestBeginBlock{Header: header})
		require.Equal(t, int64(0), sheApp.MintKeeper.GetMinter(ctx).GetLastMintAmountCoin().Amount.Int64())
	})
	...
	t.Run("even full release", func(t *testing.T) {
		...
		for i := 0; i < 25; i++ {
			...
			sheApp.EpochKeeper.BeforeEpochStart(ctx, currEpoch)
			sheApp.EpochKeeper.AfterEpochEnd(ctx, currEpoch)
			mintParams = sheApp.MintKeeper.GetParams(ctx)

			// 250k / 25 days = 100000 per day
			expectedAmount := int64(100000)
			newMinter := sheApp.MintKeeper.GetMinter(ctx)
...
func TestNoEpochPassedNoDistribution(t *testing.T) {
	...
	header := tmproto.Header{Height: sheApp.LastBlockHeight() + 1}
	sheApp.BeginBlock(ctx, abci.RequestBeginBlock{Header: header})
	// Get mint params
	mintParams := sheApp.MintKeeper.GetParams(ctx)
	genesisTime := time.Date(2022, time.Month(7), 18, 10, 0, 0, 0, time.UTC)
	presupply := sheApp.BankKeeper.GetSupply(ctx, mintParams.MintDenom)
	startLastMintAmount := sheApp.MintKeeper.GetMinter(ctx).GetLastMintAmountCoin()
	...
	for i := 0; i < 60*24*7*52-1; i++ {
		...
		currSupply := sheApp.BankKeeper.GetSupply(ctx, mintParams.MintDenom)
		require.True(t, currSupply.IsEqual(presupply))
	}
	// Ensure that EpochProvision hasn't changed
	endLastMintAmount := sheApp.MintKeeper.GetMinter(ctx).GetLastMintAmountCoin()
