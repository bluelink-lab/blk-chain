...
) ([]func(*baseapp.BaseApp), shedb.StateStore) {
	scEnabled := cast.ToBool(appOpts.Get(FlagSCEnable))
	if !scEnabled {
		logger.Info("SheDB is disabled, falling back to IAVL")
		return baseAppOptions, nil
	}
	logger.Info("SheDB SC is enabled, running node with StoreV2 commit store")
	scConfig := parseSCConfigs(appOpts)
	ssConfig := parseSSConfigs(appOpts)
	if ssConfig.Enable {
		logger.Info(fmt.Sprintf("SheDB StateStore is enabled, running %s for historical state", ssConfig.Backend))
	}
...
func validateConfigs(appOpts servertypes.AppOptions) {
	...
	if snapshotExportInterval > 0 && scEnabled {
		if !ssEnabled {
			panic(fmt.Sprintf("Config validation failed, SheDB SS store needs to be enabled when snapshot interval %d > 0", snapshotExportInterval))
		}
	}
}
