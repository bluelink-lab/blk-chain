		...
		case "increase_allowance", "decrease_allowance":
			...
			ret, err := app.WasmKeeper.QuerySmart(
				...
				[]byte(fmt.Sprintf(
					...
					app.EvmKeeper.GetSheAddressOrDefault(ctx, common.BytesToAddress(action.Owner[:])).String(),
					app.EvmKeeper.GetSheAddressOrDefault(ctx, common.BytesToAddress(action.Spender[:])).String())),
			)
			if err != nil {
				continue
			}
		...
		case "transfer_nft", "send_nft", "burn":
			...
			if ownerEvents, ok := ownerEventsMap[ownerEventKey]; ok {
				if len(ownerEvents) > currentCounter {
					ownerSheAddrStr := string(ownerEvents[currentCounter].Attributes[2].Value)
					if ownerSheAddr, err := sdk.AccAddressFromBech32(ownerSheAddrStr); err == nil {
						ownerEvmAddr := app.EvmKeeper.GetEVMAddressOrDefault(ctx, ownerSheAddr)
						sender = common.BytesToHash(ownerEvmAddr[:])
					} else {
						ctx.Logger().Error("Translate CW721 error: invalid bech32 owner", "error", err, "address", ownerSheAddrStr)
					}
				} else {
...
func (app *App) GetEvmAddressHash(ctx sdk.Context, addrStr string) common.Hash {
	sheAddr, err := sdk.AccAddressFromBech32(addrStr)
	if err == nil {
		evmAddr := app.EvmKeeper.GetEVMAddressOrDefault(ctx, sheAddr)
