...
func BlockTest(a *App, bt *ethtests.BlockTest) {
	...
	for addr, genesisAccount := range a.EvmKeeper.BlockTest.Json.Pre {
		ublt, wei := state.SplitUsheWeiAmount(genesisAccount.Balance)
		sheAddr := a.EvmKeeper.GetSheAddressOrDefault(a.GetContextForDeliverTx([]byte{}), addr)
		err := a.EvmKeeper.BankKeeper().AddCoins(a.GetContextForDeliverTx([]byte{}), sheAddr, sdk.NewCoins(sdk.NewCoin("ublt", ublt)), true)
		if err != nil {
			panic(err)
		}
		err = a.EvmKeeper.BankKeeper().AddWei(a.GetContextForDeliverTx([]byte{}), a.EvmKeeper.GetSheAddressOrDefault(a.GetContextForDeliverTx([]byte{}), addr), wei)
		if err != nil {
			panic(err)
		}
	...
	for i, btBlock := range bt.Json.Blocks {
		...
		_, err = a.FinalizeBlock(context.Background(), &abci.RequestFinalizeBlock{
			Txs:               utils.Map(b.Txs, func(tx *ethtypes.Transaction) []byte { return encodeTx(tx, a.GetTxConfig()) }),
			ProposerAddress:   a.EvmKeeper.GetSheAddressOrDefault(a.GetCheckCtx(), b.Coinbase()),
			DecidedLastCommit: abci.CommitInfo{Votes: []abci.VoteInfo{}},
			Height:            h,
			Hash:              hash,
			Time:              time.Now(),
