...
func TestPartitionPrioritizedTxs(t *testing.T) {
	...
	otherMsg := &stakingtypes.MsgDelegate{
		...
		Amount:           sdk.NewCoin("ushe", sdk.NewInt(1)),
	}
...
func TestProcessOracleAndOtherTxsSuccess(t *testing.T) {
	...
	otherMsg := &banktypes.MsgSend{
		FromAddress: account,
		ToAddress:   account2,
		Amount:      sdk.NewCoins(sdk.NewInt64Coin("ushe", 2)),
	}
	...
	oracleTxBuilder.SetFeeAmount(sdk.NewCoins(sdk.NewInt64Coin("ushe", 20000)))
	oracleTx, err := txEncoder(oracleTxBuilder.GetTx())
	require.NoError(t, err)
	...
	otherTxBuilder.SetFeeAmount(sdk.NewCoins(sdk.NewInt64Coin("ushe", 10000)))
	otherTx, err := txEncoder(otherTxBuilder.GetTx())
	require.NoError(t, err)
...
func TestDecodeTransactionsConcurrently(t *testing.T) {
	...
	bankTxBuilder.SetFeeAmount(sdk.NewCoins(sdk.NewInt64Coin("ushe", 20000)))
