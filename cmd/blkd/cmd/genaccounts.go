...
The association between the she address and the eth address will also be created here if using keyring-backend test.
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if keyringBackend == keyring.BackendTest {
				// associate the eth address with the she address through the genesis file
				evmGenState := evm.GetGenesisStateFromAppState(depCdc, appState)
				sheEthAddrAssociation := evmtypes.AddressAssociation{
					SheAddress: addr.String(),
					EthAddress: ethAddr.Hex(),
				}
				evmGenState.AddressAssociations = append(evmGenState.AddressAssociations, &sheEthAddrAssociation)
...
func getPrivateKeyOfAddr(kb keyring.Keyring, addr sdk.Address) (*ecdsa.PrivateKey, error) {
	...
	for _, key := range keys {
		...
		if localInfo.GetAddress().Equals(addr) {
			...
			// Need to use private key to convert to she address here
