...
func sendRequestWithNamespace(namespace string, port int, method string, params ...interface{}) map[string]interface{} {
	paramsFormatted := ""
	if len(params) > 0 {
		paramsFormatted = strings.Join(sheutils.Map(params, formatParam), ",")
	}
...
func formatParam(p interface{}) string {
	...
	case []common.Address:
		...
		return fmt.Sprintf("[%s]", strings.Join(sheutils.Map(v, wrapper), ","))
	case common.Hash:
		return fmt.Sprintf("\"%s\"", v)
	case []common.Hash:
	...
	case []interface{}:
		return fmt.Sprintf("[%s]", strings.Join(sheutils.Map(v, formatParam), ","))
