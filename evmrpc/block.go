...
func (a *SheBlockAPI) GetBlockByNumberExcludeTraceFail(ctx context.Context, number rpc.BlockNumber, fullTx bool) (result map[string]interface{}, returnErr error) {
	// exclude synthetic txs
	return a.getBlockByNumber(ctx, number, fullTx, false, a.isPanicTx)
}

func (a *SheBlockAPI) GetBlockByHashExcludeTraceFail(ctx context.Context, blockHash common.Hash, fullTx bool) (result map[string]interface{}, returnErr error) {
	// exclude synthetic txs
...
func (a *BlockAPI) GetBlockByNumber(ctx context.Context, number rpc.BlockNumber, fullTx bool) (result map[string]interface{}, returnErr error) {
	...
	if number == 0 {
		...
		return map[string]interface{}{
			...
			"sha3Uncles":       ethtypes.EmptyUncleHash, // inapplicable to SHE
			"logsBloom":        ethtypes.Bloom{},
			"stateRoot":        common.Hash{},
			"miner":            common.Address{},
			"difficulty":       (*hexutil.Big)(big.NewInt(0)), // inapplicable to SHE
			...
			"uncles":           []common.Hash{}, // inapplicable to SHE
...
func EncodeTmBlock(
		...
		for _, msg := range decoded.GetMsgs() {
			...
			case *banktypes.MsgSend:
				...
				if !fullTx {
					...
					senderSheAddr, err := sdk.AccAddressFromBech32(m.FromAddress)
					if err != nil {
						continue
					}
					rpcTx.From = k.GetEVMAddressOrDefault(ctx, senderSheAddr)
					recipientSheAddr, err := sdk.AccAddressFromBech32(m.ToAddress)
					if err != nil {
						continue
					}
					recipientEvmAddr := k.GetEVMAddressOrDefault(ctx, recipientSheAddr)
					rpcTx.To = &recipientEvmAddr
					amt := m.Amount.AmountOf("ushe").Mul(state.SdkUsheToSweiMultiplier)
	...
	if fullTx {
		result["totalDifficulty"] = (*hexutil.Big)(big.NewInt(0)) // inapplicable to SHE
	}
	return result, nil
}
