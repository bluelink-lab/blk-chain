...
func (t *AssociationAPI) GetSheAddress(_ context.Context, ethAddress common.Address) (result string, returnErr error) {
	...
	if !found {
		return "", fmt.Errorf("failed to find SHE address for %s", ethAddress.Hex())
	}
...
func (t *AssociationAPI) GetEVMAddress(_ context.Context, sheAddress string) (result string, returnErr error) {
	...
	ethAddress, found := t.keeper.GetEVMAddress(t.ctxProvider(LatestCtxHeight), sheAddr)
	if !found {
		return "", fmt.Errorf("failed to find EVM address for %s", sheAddress)
	}
