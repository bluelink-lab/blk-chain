...
def delegation_change(chain_id):
    ...
    for i in range(len(validator_addrs // 4)):
        ...
        shed_staking_cmd = f"blkd tx staking delegate {validator_addrs[i]} {10 * total_delegation}ublt --from admin --chain-id {chain_id} -b block -y"
    ...
    for validator in delegations:
        ...
        shed_unbond_cmd = f"blkd tx staking unbond {validator} {delegations[validator]}ublt --from admin -b block -y --chain-id {chain_id}"
...
def _run_shed_cmd(cmd)
    shed_cmd = "printf '12345678\n' " + cmd
    output = json.loads(
        subprocess.check_output([shed_cmd], stderr=subprocess.STDOUT,
                                shell=True))
    return output
