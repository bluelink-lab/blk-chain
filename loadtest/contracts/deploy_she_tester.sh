...
she_tester_res=$(printf "12345678\n" | $shedbin tx wasm store she_tester.wasm -y --from=$keyname --chain-id=$chainid --gas=5000000 --fees=1000000ublk --broadcast-mode=block --output=json)
she_tester_id=$(python3 parser.py code_id $she_tester_res)

# instantiate
echo "Instantiating..."
tester_in_res=$(printf "12345678\n" | $shedbin tx wasm instantiate $she_tester_id '{}' -y --no-admin --from=$keyname --chain-id=$chainid --gas=5000000 --fees=1000000ublk --broadcast-mode=block  --label=dex --output=json)
