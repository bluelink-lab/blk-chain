...
pub fn instantiate(
    ...
    _info: MessageInfo,
    msg: InstantiateMsg,
) -> StdResult<Response<SheMsg>> {
    MARS_ADDR.save(deps.storage, &msg.mars_address)?;
    Ok(Response::new())
}

#[entry_point]
pub fn sudo(deps: DepsMut<SheQueryWrapper>, env: Env, msg: SudoMsg) -> Result<Response<SheMsg>, StdError> {
    match msg {
...
pub fn process_settlements(
    _deps: DepsMut<SheQueryWrapper>,
    _entries: Vec<SettlementEntry>,
    _epoch: i64,
) -> Result<Response<SheMsg>, StdError> {
    Ok(Response::new())
}
