...
# store
echo "Storing..."
jupiterstoreres=$(printf "12345678\n" | $shedbin tx wasm store jupiter/artifacts/jupiter.wasm -y --from=$keyname --chain-id=$chainid --gas=5000000 --fees=1000000ushe --broadcast-mode=block --output=json)
...
printf "12345678\n" | $shedbin tx staking delegate $valaddr 1000000000ushe --from=$keyname --chain-id=$chainid -b block -y
...
jupiterinsres=$(printf "12345678\n" | $shedbin tx wasm instantiate $jupiterid '{"mars_address":"'$marsaddr'"}' -y --no-admin --from=$keyname --chain-id=$chainid --gas=5000000 --fees=1000000ushe --broadcast-mode=block  --label=dex --output=json)
jupiteraddr=$(python3 parser.py contract_address $jupiterinsres)

# register
echo "Registering..."
printf "12345678\n" | $shedbin tx dex register-contract $marsaddr $marsid false true  -y --from=$keyname --chain-id=$chainid --fees=10000000ushe --gas=500000 --broadcast-mode=block
printf "12345678\n" | $shedbin tx dex register-contract $saturnaddr $saturnid false true $marsaddr -y --from=$keyname --chain-id=$chainid --fees=10000000ushe --gas=500000 --broadcast-mode=block
...
printf "12345678\n" | $shedbin tx staking unbond $valaddr 1000000000ushe --from=$keyname --chain-id=$chainid -b block -y
