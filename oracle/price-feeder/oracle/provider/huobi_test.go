...
func TestHuobiProvider_GetTickerPrices(t *testing.T) {
	...
	t.Run("valid_request_multi_ticker", func(t *testing.T) {
		...
		tickerMap["market.sheusdt.ticker"] = HuobiTicker{
			CH: "market.sheusdt.ticker",
			Tick: HuobiTick{
		...
		prices, err := p.GetTickerPrices(
			types.CurrencyPair{Base: "ATOM", Quote: "USDT"},
			types.CurrencyPair{Base: "SHE", Quote: "USDT"},
		)
		require.NoError(t, err)
		require.Len(t, prices, 2)
		require.Equal(t, sdk.MustNewDecFromStr(strconv.FormatFloat(lastPriceAtom, 'f', -1, 64)), prices["ATOMUSDT"].Price)
		require.Equal(t, sdk.MustNewDecFromStr(strconv.FormatFloat(volume, 'f', -1, 64)), prices["ATOMUSDT"].Volume)
		require.Equal(t, sdk.MustNewDecFromStr(strconv.FormatFloat(lastPriceShe, 'f', -1, 64)), prices["SHEUSDT"].Price)
		require.Equal(t, sdk.MustNewDecFromStr(strconv.FormatFloat(volume, 'f', -1, 64)), prices["SHEUSDT"].Volume)
	})
