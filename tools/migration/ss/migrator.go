...
func (m *Migrator) Verify(version int64) error {
	...
	for _, module := range utils.Modules {
		...
		_, err = tree.Iterate(func(key []byte, value []byte) bool {
			...
			count++
			if count%1000000 == 0 {
				fmt.Printf("SheDB Archive Migration: Verified %d keys in for module %s\n", count, module)
			}
			return false
		})
		if err != nil {
			fmt.Printf("SheDB Archive Migration: Failed to iterate the tree %s: %s\n", module, err.Error())
			return err
		}
		fmt.Printf("SheDB Archive Migration:: Finished verifying module %s, total scanned: %d keys\n", module, count)
	}
	return verifyErr
}
...
func ExportLeafNodesFromKey(db dbm.DB, ch chan<- types.RawSnapshotNode, startKey []byte, startModule string) error {
	count := 0
	leafNodeCount := 0
	fmt.Println("SheDB Archive Migration: Scanning database and exporting leaf nodes...")
	...
	for _, module := range utils.Modules {
		...
		fmt.Printf("SheDB Archive Migration: Iterating through %s module...\n", module)
		...
		if err != nil {
			fmt.Printf("SheDB Archive Migration: Error creating iterator: %+v\n", err)
		...
		for ; itr.Valid(); itr.Next() {
			...
			if err != nil {
				fmt.Printf("SheDB Archive Migration: Failed to make node: %+v\n", err)
			...
			if count%1000000 == 0 {
				...
				metrics.IncrCounterWithLabels([]string{"she", "migration", "leaf_nodes_exported"}, float32(batchLeafNodeCount), []metrics.Label{
		...
		fmt.Printf("SheDB Archive Migration: Finished scanning module %s. Time taken: %v. Total scanned: %d, leaf nodes exported: %d\n", module, moduleDuration, count, leafNodeCount)
	}

	totalDuration := time.Since(startTimeTotal)
	fmt.Printf("SheDB Archive Migration: DB scanning completed. Total time taken: %v. Total entries scanned: %d, leaf nodes exported: %d\n", totalDuration, count, leafNodeCount)
