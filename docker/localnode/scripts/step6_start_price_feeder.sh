...
printf "12345678\n" | "$HOME/go/bin/blkd" tx oracle set-feeder "$ORACLE_ACCOUNT_ADDRESS" --from $VALIDATOR_ACCOUNT --fees 2000ushe -b block -y --chain-id she >/dev/null 2>&1
printf "12345678\n" | "$HOME/go/bin/blkd" tx bank send $VALIDATOR_ACCOUNT "$ORACLE_ACCOUNT_ADDRESS" --from $VALIDATOR_ACCOUNT 1000she --fees 2000ushe -b block -y >/dev/null 2>&1
