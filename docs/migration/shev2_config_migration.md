...
mempool config. For the `app.toml`, we will make sure to enable shedb and add in a whole section for evm.
...
## Full Example SheV2 config.toml
...
This is just for validator nodes. Reference `shedb_migration.md` for more information on enabling state store for rpc nodes.
...
# Pruning strategy is completely ignored when shedb is enabled
pruning = "default"

# These are applied if and only if the pruning strategy is custom, and shedb is not enabled
...
# This config only take effect when SheDB is enabled (sc-enable = true
ss-enable = true
