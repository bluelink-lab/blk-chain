...
a recent load test on SHE, P was ~600ms and Q was ~300ms, so optimistic processing would
...
cache stores, but demands special treatment for SHE's in-memory state in its `dex` module. A
...
hasn't integrated with ABCI++, SHE would need to directly integrate with ABCI++ based off
