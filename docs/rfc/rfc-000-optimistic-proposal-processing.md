...
a recent load test on BLK, P was ~600ms and Q was ~300ms, so optimistic processing would
...
cache stores, but demands special treatment for BLK's in-memory state in its `dex` module. A
...
hasn't integrated with ABCI++, BLK would need to directly integrate with ABCI++ based off
