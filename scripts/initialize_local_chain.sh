...
# gentx for account
~/go/bin/blkd gentx $keyname 7000000000000000ublt --chain-id she-chain --keyring-backend test
...
jq '.validators[0] += {"power":"7000000000"}' ~/.she/config/tmp_genesis.json > ~/.she/config/tmp_genesis_2.json
jq '.validators[0] += {"pub_key":'$KEY'}' ~/.she/config/tmp_genesis_2.json > ~/.she/config/tmp_genesis_3.json
mv ~/.she/config/tmp_genesis_3.json ~/.she/config/genesis.json && rm ~/.she/config/tmp_genesis.json && rm ~/.she/config/tmp_genesis_2.json
...
cat ~/.she/config/genesis.json | jq '.app_state["oracle"]["params"]["whitelist"]=[{"name": "ueth"},{"name": "ubtc"},{"name": "uusdc"},{"name": "uusdt"},{"name": "uosmo"},{"name": "uatom"},{"name": "ublt"}]' > ~/.she/config/tmp_genesis.json && mv ~/.she/config/tmp_genesis.json ~/.she/config/genesis.json
...
cat ~/.she/config/genesis.json | jq '.app_state["bank"]["denom_metadata"]=[{"denom_units":[{"denom":"ublt","exponent":0,"aliases":["USHE"]}],"base":"ublt","display":"ublt","name":"USHE","symbol":"USHE"}]' > ~/.she/config/tmp_genesis.json && mv ~/.she/config/tmp_genesis.json ~/.she/config/genesis.json
...
cat ~/.she/config/genesis.json | jq --arg start_date "$START_DATE" --arg end_date "$END_DATE_3DAYS" '.app_state["mint"]["params"]["token_release_schedule"]=[{"start_date": $start_date, "end_date": $end_date, "token_release_amount": "999999999999"}]' > ~/.she/config/tmp_genesis.json && mv ~/.she/config/tmp_genesis.json ~/.she/config/genesis.json
cat ~/.she/config/genesis.json | jq --arg start_date "$END_DATE_3DAYS" --arg end_date "$END_DATE_5DAYS" '.app_state["mint"]["params"]["token_release_schedule"] += [{"start_date": $start_date, "end_date": $end_date, "token_release_amount": "999999999999"}]' > ~/.she/config/tmp_genesis.json && mv ~/.she/config/tmp_genesis.json ~/.she/config/genesis.json

if [ ! -z "$2" ]; then
  APP_TOML_PATH="$2"
else
  APP_TOML_PATH="$HOME/.she/config/app.toml"
fi
...
if [ ! -z "$1" ]; then
  ...
  CONFIG_PATH="$HOME/.she/config/config.toml"
fi

if [ ! -z "$2" ]; then
  APP_PATH="$2"
else
  APP_PATH="$HOME/.she/config/app.toml"
fi
