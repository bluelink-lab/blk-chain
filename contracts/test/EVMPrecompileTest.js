...
const { setupSigners, getAdmin, deployWasm, storeWasm, execute, isDocker, ABI, createTokenFactoryTokenAndMint, getSheBalance, rawHttpDebugTraceWithCallTracer} = require("./lib");
...
describe("EVM Precompile Tester", function () {
    ...
    describe("EVM Bank Precompile Tester", function () {
        ...
        it("Fails with 'execution reverted' not 'panic occurred' when insufficient gas is provided", async function () {
            try {
                const bankSendTx = await bank.sendNative(accounts[1].sheAddress, {value: 1, gasLimit: 40000});
    ...
    describe("EVM Addr Precompile Tester", function () {
        ...
        it("Associates successfully", async function () {
            ...
            try {
                await addr.getSheAddr(unassociatedWallet.address);
            ...
            const sheAddr = await addr.getSheAddr(unassociatedWallet.address);
            expect(sheAddr).to.not.be.null;
        });
    ...
    describe("EVM Wasm Precompile Tester", function () {
        ...
        before(async function () {
            ...
            wasmContractAddress = await deployWasm(counterParallelWasm, accounts[0].sheAddress, "counter", {count: 0});
            ...
            denom = await createTokenFactoryTokenAndMint(`native-pointer-test-${random_num}`, 1000, accounts[0].sheAddress);
        });
        ...
        it("Wasm Precompile Send Coins", async function () {
            ...
            const oldBalance = await getSheBalance(wasmContractAddress);
            const oldTokenBalance = await getSheBalance(wasmContractAddress, denom);

            const oldUserTokenBalance = await getSheBalance(accounts[0].sheAddress, denom);
            ...
            const userTokenBalance = await getSheBalance(accounts[0].sheAddress, denom);
